...
export default async function handler(req) {
  ...
  try {
    ...
    const sys = `Tum ek helpful AI assistant ho — "Mera AI". ${userName ? `User: ${userName}.` : ''} ${langPrompt[language] || langPrompt.hinglish} Friendly, helpful aur concise raho.`;
    const resp = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${groqKey}` },
      body: JSON.stringify({ model: 'llama-3.3-70b-versatile', max_tokens: 1024, messages: [{ role: 'system', content: sys }, ...messages] })
    });
